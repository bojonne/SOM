...
public class DemoHexagonalLatticeMap<T extends DataVector> {
	...
	public DemoHexagonalLatticeMap(Class<T> clazz, int width, int height, Measure<T> measure, LearningFunction lfunc) throws InstantiationException, IllegalAccessException {
		this.WIDTH = width;
		this.HEIGHT = height;
	...
	public DemoHexagonalLatticeMap(T[][] map, int width, int height, Measure<T> measure, LearningFunction lfunc) {
		this.WIDTH = width;
		this.HEIGHT = height;
		this.map = map;
		this.measure = measure;
	 ...
	 */
	public boolean learn(int time, T inputData, Coordinate bmu, double threshold) {
